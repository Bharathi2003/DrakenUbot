...
async def unload(event):
    ...
    if not shortname:
        await eor(event, "`Give name of plugin which u want Draken to unload`")
        return
...
async def uninstall(event):
    shortname = event.pattern_match.group(1)
    if not shortname:
        await eor(event, "`Give name of plugin which u want Draken to uninstall`")
        return
    lsd = os.listdir("addons")
    lst = os.listdir("plugins")
    zym = f'{shortname}.py'
...
async def load(event):
    shortname = event.pattern_match.group(1)
    if not shortname:
        await eor(event, "`Give name of plugin which u want Draken to load`")
        return
    try:
        try:
            un_plug(shortname)
